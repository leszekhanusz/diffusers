...
parser = argparse.ArgumentParser(description="Generate images from prompt")
parser.add_argument(
    "--init_image",
    type=str,
    default=None,
    dest="init_image_path",
)
parser.add_argument(
    "--mask_image",
    type=str,
...
if args.init_image_path is not None:
    init_image = PIL.Image.open(args.init_image_path).convert("RGB")
...
if args.mask_image_path is not None:
    mask_image = PIL.Image.open(args.mask_image_path).convert("RGB")
    mask_image = PIL.ImageOps.exif_transpose(mask_image)
...
while True:
    ...
    with autocast("cuda"):
        image = pipe(
            ...
            num_inference_steps=args.nb_steps,
        )["sample"][0]

        image.save(folder + "/" + str(index) + ".png")
        image.show()
